...
CREATE TABLE IF NOT EXISTS `ShanShan`.`admin_info` (
  `admin_id` INT NOT NULL AUTO_INCREMENT COMMENT '管理者編號',
  `admin_account` VARCHAR(30) NOT NULL COMMENT '管理者帳號',
  `admin_name` VARCHAR(30) NOT NULL COMMENT '管理者名稱',
  `admin_password` VARCHAR(30) NOT NULL COMMENT '管理者密碼',
  `admin_account_status` INT NOT NULL COMMENT '管理者帳號狀態(0:停用1:啟用)',
...
CREATE TABLE IF NOT EXISTS `ShanShan`.`event` (
  ...
  `difficulty` INT NULL DEFAULT NULL COMMENT '難度',
  ...
  `event_cover` BLOB NULL DEFAULT NULL COMMENT '揪團封面',
  `event_status` INT NOT NULL COMMENT '揪團狀態(0:下架 1:上架 2:招募中 3:滿團 4:流團 5:成團)',
  `event_points` INT NULL DEFAULT NULL COMMENT '固定點數(會員:成團後才給點數)',
...
CREATE TABLE IF NOT EXISTS `ShanShan`.`msg_report` (
  ...
  `event_msg_id` INT NULL DEFAULT NULL COMMENT '揪團留言編號',
  `act_msg_id` INT NULL DEFAULT NULL COMMENT '活動日誌留言編號',
  `report_reason` INT NOT NULL COMMENT '檢舉原因(0:色情騷擾 1:不實資訊 2:垃圾訊息 3:仇恨言論)',
  `report_date` DATETIME NOT NULL COMMENT '檢舉日期',
  `case_done` DATETIME NULL DEFAULT CURRENT_TIMESTAMP COMMENT '結案時間',
  `case_status` INT NOT NULL COMMENT '案件狀態(0:待處理 1:未處理 2:已完成)',
